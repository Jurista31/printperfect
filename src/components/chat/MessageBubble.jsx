...
export default function MessageBubble({ message }) {
    ...
    return (
        <div className={cn("flex gap-3", isUser ? "justify-end" : "justify-start")}>
            {!isUser && (
                <div className="h-8 w-8 rounded-full bg-gradient-to-br from-cyan-500 to-teal-500 flex items-center justify-center mt-0.5 flex-shrink-0">
            ...
            <div className={cn("max-w-[85%]", isUser && "flex flex-col items-end")}>
                {message.content && (
                    <div className={cn(
                        ...
                        isUser ? "bg-gradient-to-br from-cyan-600 to-teal-600 text-white" : "bg-slate-800/50 text-slate-100 border border-slate-700"
                    )}>
                        {isUser ? (
                                    ...
                                    code: ({ inline, className, children, ...props }) => {
                                        ...
                                        return !inline && match ? (
                                            <div className="relative group/code">
                                                <pre className="bg-slate-950 text-slate-100 rounded-lg p-3 overflow-x-auto my-2 border border-slate-800">
                                            ...
                                            <code className="px-1.5 py-0.5 rounded bg-slate-700/50 text-cyan-300 text-xs">
                                                {children}
                                            </code>
                                        );
                                    },
                                    a: ({ children, ...props }) => (
                                        <a {...props} className="text-cyan-400 hover:text-cyan-300 underline" target="_blank" rel="noopener noreferrer">{children}</a>
                                    ),
